...
function buildVolumeMounts(
  ...
  // gogcli credentials directory (for Google Workspace tools inside the container)
  const gogcliDir = path.join(homeDir, '.config', 'gogcli');
  if (fs.existsSync(gogcliDir)) {
    mounts.push({
      hostPath: gogcliDir,
      containerPath: '/home/node/.config/gogcli',
      readonly: false, // gogcli may need to refresh OAuth tokens
    });
  }

  // gh CLI credentials directory (for GitHub operations inside the container)
